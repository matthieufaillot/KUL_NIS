...
function KUL_hd_glio_auto {
    ...
    if [ ! -f ${hdgliooutputdir}/output/segmentation.nii.gz ]; then
        ...
        if [ ! -f /usr/local/KUL_apps/HD-GLIO-AUTO/scripts/run.py ]; then
            task_in="docker run --runtime=nvidia --gpus all --mount type=bind,source=$hdglioinputdir,target=/input \
...
if [ ! -f $fastsurferoutputdir/$participant/mri/aparc.DKTatlas+aseg.deep.mgz ]; then
  kul_echo "Running segmentation-only fastsurfer"
  mkdir $fastsurferoutputdir
  singularity exec --nv\
  -B $bidsdir:/data \
  -B $fastsurferoutputdir:/output \
  -B $KUL_apps_DIR/freesurfer_license:/fs_license \
  -B $FASTSURFER_HOME \
  $KUL_apps_DIR/fastsurfer-gpu.sif \
  $FASTSURFER_HOME/run_fastsurfer.sh \
  --t1 $cwd/$T1w \
  --sd $fastsurferoutputdir \
  --sid $participant \
  --seg_only --py python --ignore_fs_version
  #KUL_task_exec $verbose_level "Running FastSurfer" "Fastsurfer"
